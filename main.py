...
def main(args):
    ...
    mainBrowserTool = BrowserTool(default_site=args.url,proxyHost=args.proxy_host,proxyPort=args.proxy_port, username=args.username, password=args.password)
...
def open_another_browser(default_site="https://github.com/tristandostaler/CTFTool", proxyHost="127.0.0.10", proxyPort="8080", username="", password=""):
    global allBrowserTool
    browserTool = BrowserTool(default_site,proxyHost,proxyPort,username,password)
...
def parseArgs():
    ...
    parser.add_argument("-u", "--url", action="store_true", default="https://github.com/tristandostaler/CTFTool",
                    help="The default website to open on load")
    parser.add_argument("-o", "--proxy-host", action="store_true", default="127.0.0.1",
                    help="The proxy host to use. Ignored when --no-proxy")
    parser.add_argument("-p", "--proxy-port", action="store_true", default="8080",
                    help="The proxy port to use. Ignored when --no-proxy")
    parser.add_argument("--no-proxy", action="store_true",
                    help="Switch. Used to ignore proxy settings")
    parser.add_argument("-i", "--username", action="store_true", default="",
                    help="Optional. Username to use when calling the login function")
    parser.add_argument("-d", "--password", action="store_true", default="",
